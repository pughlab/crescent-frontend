...
const useSecondaryRunEvents = () => {
   ...
   */
  const cancelSecondaryRunInit = useCallback(({ cancelFunction }) => {
    send({
   ...
   */
  const cancelSecondaryRun = useCallback(({ cancelOptions }) => {
    send({
   ...
   */
  const updateLogs = useCallback(({ logs }) => {
    send({
      type: 'UPDATE_LOGS',
      logs
    })
   ...
   */
  const updateStatus = useCallback(({ secondaryRunWesID, status }) => {
    send({
   ...
   */
   const uploadInput = useCallback(({ inputIndex, uploadOptions }) => {
    send({
      type: 'UPLOAD_INPUT',
      inputIndex,
      uploadOptions
