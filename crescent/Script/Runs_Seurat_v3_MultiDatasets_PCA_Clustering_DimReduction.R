...
suppressPackageStartupMessages(library(jamba))        # (GitHub) to do alphanumeric sorting. Only needed if using `-w Y`.
...
if (regexpr("^Y$", RunsCwl, ignore.case = T)[1] == 1) {
  ...
  features_tsv_ordered <- as.data.frame(mixedSort(features_tsv$features))
