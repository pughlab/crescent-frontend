...
Tempdir        <- "SEURAT" ## Using this for temporary storage of outfiles because sometimes long paths of outdirectories casuse R to leave outfiles unfinished
...
png(file=paste(Tempdir,"/",PrefixOutfiles,".SEURAT_VariableGenes.png", sep=""))
...
png(file=paste(Tempdir,"/",PrefixOutfiles,".SEURAT_PCHeatmap.C1to",PCHeatmapComponentsToPlot,".png", sep=""))#, width=7, height=12)
...
# changed to png for crescent portal mockup
...
if (regexpr("^NA$", ListGenes, ignore.case = T)[1] == 1) {
  ...
  png(file=paste(Tempdir,"/",PrefixOutfiles,".SEURAT_TSNEPlot_SelectedGenes.png", sep=""), width=pdfWidth, height=pdfHeight)
